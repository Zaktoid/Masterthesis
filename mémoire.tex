...
\todo{cette section est à restructurer et à retravailler, et complétée}\\
Dans cette section nous rappelons brièvement les résultats classiques du calcul différentiel dans des espaces de Banach. Les plus importants seront démontrés en détail et afin de ne pas alourdir le manuscrit, nous renvoyons le lecteur vers \cite{dieudonne1969fma8} pour les preuves détaillées des autres.

\begin{definition}
Soient $E,F$ des espaces de Banach (réels ou complexes) et $U$ un ouvert de $E$ et $f,g\in C^0(E,F)$. $f$ et $g$ sont \textit{tangents} en $x_0\in A$ si 
\begin{equation*}
    \lim_{x\rightarrow{x_0},x\neq x_0}  \frac{||f(x)-g(x)||_F}{||x-x_0||_E}=0
\end{equation*}
\end{definition}
Il est clair que le relation "être tangent en un point" est une relation d'équivalence.
\begin{lemme}
Dans les conditions de la définition précédente, si il existe un élément $h$ de $C^0(A,F)$ tangent à $f$ en $x_0$ tel qu'il existe il existe $u\in L(E,F)$ tel que
\begin{equation}
    h(x)=f(x_0)+u(x-x_0)
\end{equation}
alors il est unique
\end{lemme}
\begin{preuve}
   Supposons avoir $h$ et $h'$ satisfaisant $(1)$ et notons $u_1,u_2$ les éléments de $L(E,F)$ correspondant, on a alors 
   \begin{equation*}
           \lim_{x\rightarrow{x_0},x\neq x_0}  \frac{||h(x)-h'(x)||_F}{||x-x_0||_E}
           = \lim_{x\rightarrow{x_0},x\neq x_0}  \frac{||u_1(x)-u_2(x)||_F}{||x-x_0||_E}
           =           \lim_{y\rightarrow{0},y\neq 0}  \frac{||v(y)||_F}{||y||_E}=0
   \end{equation*}
...
\begin{proposition}
    Dans les conditions de la définition précédente, $Df(x_0)\in \mathcal{L}(E,F)$
\end{proposition}
\begin{preuve}
    Pour tout $\varepsilon>0$, il existe $0<r<1$ tel que
    \begin{equation*}
        ||t||_E<r\Rightarrow ||f(x_0+t) - f(x_0)||_F\leq 
        \frac{\varepsilon}{2}\text{ et } ||f(x_0+t)-(f(x_0)+u(t))||_F\leq\frac{\varepsilon}{2} ||t||_E
    \end{equation*}
Ainsi, $||t||\leq r\Rightarrow ||u(t)||\leq\varepsilon$,ce qui permet de conclure.
\end{preuve}
On vérifie facilement que si $f$ est constante sur $A$, alors $f$ est dérivable sur $A$ et que celle-ci y est nulle. De même, il est immédiat que si $u\in \mathcal{L}(E,F)$ alors $u$ est dérivable et $Du(x) =u(x)$ sur $E$.
\begin{remarque}
...
\end{remarque}
\begin{remarque}
On pourrait également se demander "pourquoi $\mathbb{R}$ ou $\mathbb{C}$ ? N'est-il pas possible de travailler avec des champs plus généraux ?". D'une part, notons d'abord qu'il faut pouvoir définir une notion de limite intéressante, ce qui exclue d'emblée les champs finis. Il est possible de faire de l'analyse avec les p-adique, mais la "rigidité" de leur topologie fait qu'il n'est pas vraiment intéressant d'y étudier les bifurcation. Le lecteur intéressé par l'analyse dans des champs plus généraux pourra consulter [Source]. 
\end{remarque}
Rappelons que l'espace $\mathcal{L}(E,F)$ muni de la norme $||u||=\sup_{t\in B(1)} u(x)$  est un espace de Banach et que $|| u(x) ||_F\leq ||u||\cdot||x||_E$. 

\begin{proposition}
Si $E,F,G$ sont des espaces de Banach et si $B\in C^0(E\times F,G)$ est 
bilinéaire, alors B est dérivable et $DB(x_0,y_0)(x,y)= B(x_0,y)+B(x,y_0)$ 
pour tous $(x_0,y_0),(x,y)\in E\times F$
\end{proposition}
\begin{preuve}
     Par hypothèse, il existe $C>0$ tel que $||B(x,y)||_G\leq ||x||_E ||y||_F$. Et donc, si $\varepsilon>0$ est fixé et si $\sup(||s||_E,||t||_F)=||(s,t)||_{E\times F}\leq \frac{\varepsilon}{C}$, on a 
     \begin{equation*}
         || B(x_0+x,y_0+y)-B(x_0,y_0)-
         B(x_0,y)-B(x,y_0)||_F= ||B(x,y)||_F\leq\varepsilon ||(x,y)||
     \end{equation*}
\end{preuve}

On obtient facilement le résultat suivant:
\begin{proposition}
    Si $(F_i)_{i=0}^m$ est une suite finie d'espaces de Banach,$A$ un ouvert d'un espace de Banach $E$ , $F=\prod_{i=0}^m F_i$ 
    et si $f\in C^0(A,F)$ alors, pour tout $x_0\in E$
    \begin{equation*}
    f \text{ est dérivable en $x_0$ }
    \Leftrightarrow \forall 0\leq i \leq m\quad  \pi_i(f) \text{ est dérivable en $x_0$} 
    \end{equation*}
\end{proposition}
On écrira $Df(x_0)=(Df(x_0)_1,\cdots,Df(x_0)_m)$ où $f_i=\pi_i(f)$
\\
On récupère alors le théorème de dérivation des fonctions composées. La preuve est assez similaire au cas $\mathbb{R}^n$
\begin{proposition}
    \begin{equation*}
        \forall E,F,G \text{ espaces de Banach},x_0\in E, A\in\mathcal{V}_{x_0},f\in C^0(A,F),y_0\in F ,B\in \mathcal{V}_{y_0},g\in C^0(B,G)
    \end{equation*}
    \begin{equation*}
        f\text{ dérivable en  $x_0$ et g dérivable en $y_0$ et  $f(x_0)=y_0$}\Rightarrow g\circ f \text{ dérivable en $x_0$}  
    \end{equation*}
...
\begin{remarque}\label{remarque 1-dim}
    Avec les notations qui précèdent, si $E$ a une seule dimension (il est dans ce cas isomorphe à son champ de scalaire), $\mathcal{L}(E,F)$ est isomorphe à $F$ via $f\mapsto (\xi\mapsto f\xi)$
    $Df(x_0)$ est alors un élément de l'espace d'arrivée $F$
\end{remarque}
    \begin{proposition}
        Soit $I=[\alpha,\beta]$ un intervalle compact de $\mathbb{R}$,  $F$ un espace de banach, $f\in C^0(I,F)$,$\varphi\in C^0(I,\mathbb{R})$, supposons que $D\subset I$ soit dénombrable, et que $\varphi$ et $f$ soient dérivables sur $I\setminus D$ et que
    $||Df||\leq ||D\varphi||$ sur $I$ alors $||f(\beta)-f(\alpha)||\leq \varphi(\beta)-\varphi(\alpha)$
\end{proposition}
\begin{preuve}
    \todomaybe{écrire la preuve}
\end{preuve}
On obtient immédiatement le corollaire suivant
\begin{corollaire}
dans les conditions de la proposition précédente, si $||Df||\leq M$ sur  alors
$||f(\beta)-f(\alpha)||\leq M(\beta-\alpha)$
\end{corollaire}
Comme conséquence naturelle, le résultat suivant
\begin{proposition}
    Soient $E,F$ des espaces de Banach,
    $f$ une fonction continue entre $I=\{x_0+st|s\in[0,1]\}$
    où $x_0,t$ sont des éléments de $E$.
    Si $f$ est dérivable sur $I$ , alors
    \begin{equation*}
        ||f(x_0+t) -f(x_0)||\leq ||t||\cdot \sup_{s\in[0,1]}||D_{x_0+ts}f||
    \end{equation*}
\end{proposition}
\begin{preuve}
    \todomaybe{ écrire la preuve}
\end{preuve}
\begin{corollaire}
    Soient $E,F$ des espaces de Banach, soient $a,b$ des points de $E$ et $I$ un segment les joignant, $A$ un ouvert de $F$ et $f\in C^0(I,A)$ dérivable sur $I$, on a 
    \begin{equation*}
        ||f(b)-f(a)- Df(x_0)(b-a) ||
        \leq ||b-a||\cdot \sup_{x\in I} ||Df(x)-Df(x_0)||
    \end{equation*}
\end{corollaire}
\begin{definition}
    Soient $E,F$ des espaces de Banach, soit $A$ un ouvert de $E$.
    $f\in C^1(A,F)$ si $Df\in C^0(A,\mathcal{L}(E,f))$
\end{definition}
\begin{definition}
    Dans les conditions de la définition précédente, si $E=\prod_{i=1}^n E_i$ où les $E_i$ sont des espaces de Banach. 
    $x_i\mapsto f(a_1,\cdots,x_i,\cdots,a_n)$
    est dérivable. On la note  $D^{(i)}_{a} f$
\end{definition}
\begin{proposition}
    Dans les conditions des définitions précédentes, 
    \begin{equation*}
        f\in C^1(A,F)\Leftrightarrow \forall a\in A, \space
        \forall 1\leq i \leq n \quad  D^{(i)}_a f\text{ est défini et continu}
    \end{equation*}
    la dérivée est alors donné par
    \begin{equation*}
        Df(x)(t)=\sum_{i=1}^n D_i f(a)(t)
    \end{equation*}
\end{proposition}
\begin{preuve}
    voir \cite{dieudonne1969fma8}
...
\begin{theorem}
Soient $X,\Lambda,Y$ des espaces de Banach, $U$ un ouvert de $X\times \Lambda$ et $f\in C^1(A,G)$. Soit $(x_0,\lambda_0)\in Z_Af$ tel que $D_1 f(x_0,y_0)$ est un homéomorphisme linéaire de $\Lambda$ vers $Y$, alors il existe  $U_0\in \mathcal{V}_{a_0}$ tel que pour 
\begin{equation*}
    \forall U\in\mathcal{P}(U_0)\cap\mathcal{V}_{a_0} \text{ connexe, }\exists ! u\in C^0(U,F) :\bigg( u(a_0)=b_0\text{ et  } \forall x\in U , (x,u(x))\in A ,f(x,u(x))=0 \bigg)
\end{equation*}
De plus,
\begin{equation*}
    u\in C^1(U,F)  \text{ et } Du(x)=-(D_{1}f(x,u(x)))^{-1}\circ 
    D^{(2)}_{(x,u(x))}f
\end{equation*}
\end{theorem}
\begin{preuve}
...
\todo{Rajouter une partie sur les fonctions analytiques dans des Banachs}
\\

\todo{Inverse local analytique}
\\

On peut maintenant poser proprement le problème général de la théorie de la bifurcation.
Etant donné un espace d'état $X$, un espace de paramètre $\Lambda$ et une fonction $F:X\times \Lambda\rightarrow{ Y}$ (où $X$,$\Lambda$,$Y$ sont des espaces Banach réels). Considérons maintenant $(x_0,\lambda_0)$ 
tel que 
\begin{equation}
    F(x_0,\lambda_0)=0
\end{equation}
Si $D^{(1)}_{(x_0,\lambda_0)}f$ est un homéomorphisme linéaire, alors le problème est entièrement réglé par le théorème précédent. On s'intéresse alors aux situations où $D^{(1)}_{(x_0,\lambda_0)}f$ n'est pas inversible sur un voisinage de $(x_0,y_0)$.
\newpage
\section{Notions de théorie des opérateurs}
...
\begin{lemme}\label{lemma-compo}
    Soint $E,F,G$ des espaces Banach, $D_1:E\rightarrow F, D_2: F\rightarrow G$ si 2 des 3 opérateurs $D_1$,$D_2$ et $D_1\circ D_2$ est de Fredholm alors le 3ème l'est aussi et on a 
    \begin{equation*}
...
\begin{equation*}
    \im(D_1\circ D_2)\subseteq \im(D_1)\subseteq G
\end{equation*}
...
\begin{equation*}
    \dim(G/\im(D_2\circ D_1)) = \dim(G/\dim D_2) + \dim(F/\im D_1) - \dim(\ker(D_2)) + \dim(\ker(D_2)\cap \im(D_1))
\end{equation*}
...
\begin{preuve}
    ...
    Montrons alors que $D'$ est l'inverse de $D$.
    On a
    \begin{equation*}
    ...
    En posant $S_k=\sum_{0\leq n\leq k} D^k$
    \begin{equation*}
...
des ouverts, $F\in C^1(U\times V,Y)$ , $(x_0,\lambda_0)\in U\times V$ tel que $F(x_0,\lambda_0)=0$ et tel que $D_1F(x_0,\lambda_0)$ est de fredholm. Il existe $U_1\times V_1 \in\mathcal{V}_{(x_0,\lambda_0)}$ , $U_2$ un ouvert de $\ker D$ et 
...
\begin{preuve}
    ...
    Posons alors
    \begin{equation*}
        \psi : \ker D\times X_0\rightarrow X\times \Lambda , (v,w)\mapsto (v+w,\lambda) \text{ et } \tilde{F}(v,w,\lambda)= F(\psi(v,w,\lambda))
    \end{equation*}
...
\end{equation*}
Ainsi,  $D_2G(v_0,w_0,\lambda_0)$ est bijectif, ce qui nous permet d'appliquer le théorème des fonctions implicites, i.e , il existe un voisinnage $U_0$ de $(v_0,\lambda_0)$ dans $U_2\times V$ une fonction $\varphi\in C^1(U_1\times V_1,W_2)$ telle que $\varphi(v_0,\lambda_0)=w_0$
\begin{equation*}
...
\end{equation}
répondent à la quesiton
\end{preuve}
On peut interpréter ce résultat de la façon suivante: lorsque l'opérateur dérivée est de Fredholm, le lieu d'annulation de $F$ est en bijection avec le lieu d'annulation d'une
Des calculs simples nous peremettent d'obtenir
\begin{corollaire}
    Avec les notations de la preuve précédente, 
    \begin{equation*}
        D\varphi(v)(v_0,\lambda_0)=0\text{ et }    D\Phi(v)(v_0,\lambda_0)=0
    \end{equation*}
\end{corollaire}
...
\begin{proposition} \textit{(Fold)} \label{fold}
    ...
    \end{equation*}
    Telle que $\gamma(0,0)=(x_0,\lambda_0)$
    \begin{equation*}
...
\begin{preuve}
    ...
    \end{equation*}
    or, étant donné que $D_2F(x_0,\lambda_0)\notin \im(D_1F(x_0,\lambda_0))$, la projection doit être différente de zéro ! On applique alors le théorème des fonctions implicites, et donc, quitte à reduire $U_1\times V_1$, on peut supposer l'existance d'une fonction 
    \begin{equation*}
    ...
    \begin{equation*}
        x_1\in X : ||x_1||=1 \text{ et } \ker D_1 F(x_0,\lambda)=\rangle x_1\langle_{\R}
    \end{equation*}
    Soit $u_0\in U_1$, il existe $\delta>0$ tel que $u_0+t x_0\in U_1$ si $|t|<\delta $ 
    et 
    \begin{equation*}
        \gamma : \left] -\delta,\delta\right[\rightarrow X\times \R ,t\mapsto
                    \begin{pmatrix}
                    u_0+t x_1+\varphi(u_0+tx_1,\psi(u_0+tx_1)) \\
                    \psi(u_0+tx_1)
                    \end{pmatrix}
    \end{equation*}
...
\begin{theorem}(Crandall-Rabinowitz, bifurcation simple) 
    ...
    \begin{itemize}
        ...
        avec $||x_1||=1$ et on impose alors \\ $D^2_{12} F(0,\lambda_0)x_0\notin \im D_1 F
        (0,\lambda_0)$
    \end{itemize}
...
Il est possible d'obtenir quelques résultats quand la dimension de l'espace de paramètre est infinie. Notez que ça n'a pas été très souvent fait, car en général les paramètres sont iterprétés comme une "perturbation" du système dynamique (rappelez vous que souvent, la fonct est un genre de potentiel,  et l'annuler revient à trouver des points d'équilibres). Nous présentons alors quelques résultats de [SOURCE].
...
\begin{preuve}
    ...
    Supposons le résultat établit pour $n\leq m$ il vient alors
    \begin{equation*}
        f\in\mathcal{M}_n^{m+1}\Leftrightarrow 
        \exists n\in\mathbb{N} , g_1,\cdots,g_n \in \mathcal{M}_n^{m}: f=\sum_{j=1}^nx_j g_j
    \end{equation*}
...
\begin{equation*}
    J^k(n,m)\cong \mathcal{E}_n /\mathcal{M}_n^{m+1}
\end{equation*}
