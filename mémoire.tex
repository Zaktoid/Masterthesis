...
\end{definition}
Dans la suite de cette section, nous suppons $F$ $C^1$ sur un ouvert contenant $(x_0,\lambda_0)$ et  $\operatorname{Ind}(D_1F(x_0,\lambda_0))=0$ (c.f remarque \ref{remarque IND=0})
\begin{definition}
    Avec les notations habituelles, si $F$ est $C^1$, on note 
    \begin{equation*}
        \alpha_{F}(x_0,\lambda_0)= \dim \big(\ker (D_2 F(x_0,\lambda_0)^{*})\cap ker (D_1 F(x_0,\lambda_0)^{*})\big)
    \end{equation*}
\end{definition} 
...
\begin{definition}
    ...
    \begin{equation*}
        \dim \ker D_1 F(x_0,\lambda_0)=n=\dim \ker(D_1 F^{*})\qquad  \text{et}\qquad   \alpha_F(x_0,\lambda_0)=\alpha
    \end{equation*}
\end{definition}

Considérons alors 
\begin{equation*}
    x_1,\cdots,x_n\in X : \rangle x_1,\cdots,x_n\langle_{\R}=\ker(D_1 F(x_0,\lambda_0))
\end{equation*}
et, 
\begin{equation*}
     y^{*}_1,\cdots,y^{*}_n\in Y^{*}: \rangle y^{*}_1,\cdots, y^{*}_{n}  \langle_{\R}=\ker(D_1 F(x_0,\lambda_0)^{*})
\end{equation*}
...
\begin{proposition}
    $(x_0,\lambda_0)$ est un zéro de type $(0,n)$ si et seulement si les $D_2 F(x_0,\lambda_0)^{*}y_j^{*}$ sont linéairement indépendants
\end{proposition}
\begin{preuve}
    Supposons d'abord que $\alpha=0$ et que les $D_2 F(x_0,\lambda_0)^{*}y_j^{*}$ sont linéairement dépendants,
    on a alors 
    \begin{equation*}
        \exists c_1,\cdots,c_n\in\R: \sum_{i=1}^n D_2 F(x_0,\lambda_0)^{*}y_j^{*}=0\text{ et } \sum_{i=1}^n |c_j|>0
    \end{equation*}
    et donc du fait que les $y^{*}_j$ sont linéairement indépendants, on  a 
    \begin{equation*}
        D_2 F(x_0,\lambda_0)^{*} \Bigg(\sum_{i=1}^n y_j^{*}\Bigg)=0\text{ et } \sum_{i=1}^n c_jy_j^{*}\neq 0\Rightarrow  \ker (D_2 F(x_0,\lambda_0)^{*})\cap ker (D_1 F(x_0,\lambda_0)^{*})\neq {0}
    \end{equation*}
    une contradiction.\\
