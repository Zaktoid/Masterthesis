...
\begin{preuve}
    ...
    On remarque que, en posant $v=\pi_1(x)$, $w=(I-\pi_1)(x)$
...
et
\begin{equation*}
    H: Z_{\Phi}\rightarrow Z_F, (x,\lambda)\mapsto (v+\varphi(v,\lambda),\lambda)
\end{equation*}
...
\begin{exemple}
   ...
   la  première composante (souvenez-vous qu'on projette sur le co-noyau). Dans ce cas-ci, le système s'écrit  \eqref{SysteLP} simplement
   \begin{align*}
    x\sin(3x)-y=0\\
    \lambda x -x^3+xy^2=0
   \end{align*}
...
\begin{exemple}\label{Poutre}
    ...
    \end{equation*}
    ici, $\cdot^\perp$ est l'orthogonal dans $L^2$. Mais étant donné qu'on a un problème elliptique avec des conditions de Neumann au bord, l'indice de Fredholm est zéro, l'ensemble image est donc exactement $\phi_n^{\perp}$. On a donc des choix très naturels pour les projections: 
    \begin{equation*}
    ...
    \end{equation*}
    On pourrait alors calculer la forme réduite et décrire la géométrie locale en utilisant le théorème des fonctions implicites et en appliquant un developpement limité. Cependant, un résultat qui suit permet d'obtenir directement le forme du lieu d'annulation.
\end{exemple}
...
\begin{preuve}
    ...
    \end{align*}
    En évaluant cette expression en $(0,\lambda_0)=0$, et via \ref{corr-deriv-o}, le premier terme s'annule. Au vu de la définition de $\pi_2$, le second s'annule aussi. Et donc, encore via \ref{corr-deriv-o} et par hypothèses on a
    \begin{equation*}
        D_{\lambda}\tilde{\Phi}(0,\lambda_0)=\pi_2 D^2_{12}F(0,\lambda_0)x_1\neq 0
    \end{equation*}
    la dérivée passant dans l'intégrale car $\Phi$ est $C^2$.
    de façon similaire à \ref{fold}, on applique le théorème des fonctions implicites, donc quitte à réduire l'intervalle $]-\delta,+\delta[$ on peut supposer l'existance d'une fonction 
        \begin{equation*}
...
La courbe
    \begin{equation*}
        \gamma : \left] -\delta,\delta\right[\rightarrow X\times \R,s\mapsto
                    ...
                    \end{pmatrix}
    \end{equation*}
Répond à la question.  En effet, supposon avoir $(sx_1,\lambda)\in U_1\times V_1$ tel que $\Phi(sx_1,\lambda)=0$. si $s=0$, alors on est sur la branche triviale. sinon, on a
\begin{equation*}
...
\end{remarque}
\begin{exemple}
    Continuons l'exemple \ref{Poutre}
    \todo{vérifier les conditons et appliquer CR}
\end{exemple}
...
\begin{definition}
    ...
    $F(x_i,\lambda_i)=F(x'_i,\lambda'_i)=0$ pour tout $i\in\mathbb{N}$. Un élément de bifurcation est dit \coldef{perpendiculaire} s'il existe une suite de $X$ $(x_i)_i$ qui tend vers $x_0$ telle que $F(x_i,\lambda_0)=0$ et $x_i\neq x_0$ pour tout $i\in\mathbb{N}$
\end{definition}
\begin{remarque}
    Remarquez qu'une bifurcation depuis la branche triviale est un élément de bifurcation mais que l'inverse n'est pas vrai.
\end{remarque}
dans la suite de cette section, nous supposons $F\in C^k (U\times V)$ ($k\geq 1$) sur un ouvert contenant $(x_0,\lambda_0)$ et  $\operatorname{Ind}(D_1F(x_0,\lambda_0))=0$ (c.f remarque \ref{remarque INd=0}). Posons également $D_1=D_1F(x_0,\lambda_0)$ et $D_2=D_2F(x_0,\lambda_0)$
...
\begin{proposition}\label{ApplicationFI}
    Si $(x_0,\lambda_0)$ est un zéro de type $(0,n)$ alors il existe $0 \in V_1$ un ouvert de $\Lambda$ et $U_1$ un ouvert de $X\times \Lambda\ni (x_0,\lambda_0)$
    \begin{align*}
...
\begin{corollaire}
    Dans les conditions de la proposition précédente, et en exigeant $F$ au moins $C^2$. Si on pose $T(\mu)=\mu-\sum_{i=1}^n (D_2^{*}y_i^{*})(\mu)\lambda_i$
    \begin{align*}
        W: \Lambda\rightarrow Y: \mu\mapsto 
        &D^2_{22}F\bigg(T(\mu),T(\mu)\bigg)
        +2D_{21}^2F\bigg(T(\mu), U^{-1}\big(D_2\mu\big) \bigg)\\
        &+D^2_{11}F\bigg( U^{-1}\big(D_2\mu\big),U^{-1}\big
        (D_2\mu\big)\bigg)
    \end{align*}
    Et  
    \begin{equation*}
...
\begin{preuve}
    ...
    \end{equation*}
    on pose alors $v^{+}_n=\sqrt{2u_ny_1^{*}(D^2_{11}F(x_1,x_1))^{-1}}$ et $v^{-}_n=-\sqrt{2u_ny_1^{*}(D^2_{11}F(x_1,x_1))^{-1}}$. Bien sûr, $v^{-}_n\neq v^{+}_n $ pour tout $n\in\mathbb{N}$, posons également 
    \begin{equation*}
        \lambda_n = \lambda_0 -g(\Phi(v_n))\lambda_1
    \end{equation*}
