...
Dans sa forme la plus générale, la théorie de la bifurcation (statique) pourrait se résumer aux différentes techniques mises en place pour répondre à la question suivante : étant donné deux espaces abstraits $X$ et $Y$, un espace de paramètres $\Lambda$ et une famille de fonctions 
\begin{equation*}
...
Dans cette section, nous rappelons brièvement les résultats classiques du calcul différentiel dans des espaces de Banach. Le but est de pouvoir présenter le \colprop{théorème des fonctions implicites}, un des outils principaux de la théorie de la bifurcation. Nous nous basons principalement sur \cite{dieudonne1969fma8} et \cite{coleman2012calculus}. Dans la suite, les lettres $X,Y,\Lambda,Z$ font référence à des espaces de Banach.
...
$F\in C^0(U,Y)$ est \coldef{dérivable} en $x_0\in U$ s'il existe $u\in L(X,Y)$ tel que  $h(x)=f(x_0)+u(x-x_0)$ soit tangent à $F$ en $x_0$, ce $u$ est alors \coldef{la dérivée de $F$ en $x_0$} qu'on notera $DF(x_0)$. Naturellement, $F$ est dérivable sur $U$ si elle est dérivable en chaque point.
\end{definition}
...
\end{remarque}
\begin{remarque}
Nous allons travailler principalement avec des espaces de Banach réels dans le reste de ce travail. Il existe également des résultats dans le cas complexe. Quant aux autres champs ($p$-adiques, ...), il ne semble pas qu'il y ait une théorie des bifurcations  bien établie. Notez cependant qu'il existe une généralisation du théorème des fonctions implicites à d'autres champs ! Voir par exemple \cite{glockner2006implicit}
\end{remarque}
...
\begin{proposition}
    ...
    \end{equation*}
\end{proposition}
On écrira $DF(x_0)=(DF_0(x_0),\cdots,DF_m(x_0))$ où $F_i=\pi_i(F)$
\\
...
\begin{proposition}
    ...
    \begin{equation*}
        F\text{ dérivable en  $x_0$ et G dérivable en $y_0$}\Rightarrow G\circ F \text{ dérivable en $x_0$}  
    \end{equation*}
...
\begin{remarque}\label{remarque 1-dim}
    ...
    $Df(x_0)$ est alors identifié un à élément de l'espace d'arrivée $Y$
\end{remarque}
...
\begin{definition}
    ...
    de Banach, \\si $a=(a_1,\cdots,a_n)\in\prod_{i=1}^n X_i$,  
...
\begin{proposition}
    ...
    \begin{equation*}
        f\in C^1(U,Y)\Leftrightarrow
        \forall 1\leq i \leq n \quad  D_i f:U\rightarrow \mathcal{L}(X_i,Y)\text{ est défini et continu}
    \end{equation*}
...
\begin{corollaire}
    Soient $U$ un ouvert de $X$ et $x_0\in U$. Soit de plus $F\in C^1(U,Y)$.
    Si $DF(x_0)$ est un isomorphisme alors il existe $U'\subseteq U$ un voisinage ouvert de $x_0$ et $V'$ un voisinage de $y_0$ tels que
    $F|_{U'}\in C^1(U',V')$ soit un difféomorphisme. On a de plus
    \begin{equation*}
        D(F|_{U'}^{-1})(y)= DF(F|_{U'}^{-1}(y))^{-1}
    \end{equation*}
\end{corollaire}
\begin{preuve}
    Appliquer le résultat précédent à  $F(x)-y$
\end{preuve}
...
\begin{definition}
    Soient $X,Y$ des espaces de Banach et soit $D\in  \mathcal{L}(X,Y)$. \coldef{l'indice de  Fredholm de $D$} est défini si $\dim \ker D$ et $\dim  Y / \text{im } D=\text{codim } \text{im }D$ sont finis, et il vaut, dans ce cas
    \begin{equation*}
...
\begin{definition}
    ...
    $\mathcal{F}_n(X,Y)$  l'ensemble des opérateurs de Fredholm d'indice $n$ et $\mathcal{F}(X,Y)$ l'ensemble des opérateurs de Fredholm. On dit qu'une fonction $F: X\rightarrow Y$ est \coldef{de Fredholm en } $x\in X$ si elle est dérivable en $x$ et si $DF(x)$ est de Fredholm. Une fonction est de Fredholm sur un ouvert $U$ de $X$ si elle est $C^1$ sur $U$ et si sa dérivée est de Fredholm sur $U$, si l'indice de la dérivée est constant sur $U$ alors l'indice de $F$ est simplement l'indice de la dérivée en un point
\end{definition}
...
\begin{equation*}
    A^{\perp}=\left\{ \varphi\in Y^{*} |\varphi_{|_A}=0\right\}
\end{equation*}
\begin{exemple}
dans la suite, on munit $C^k([0,1])$ de la norme habituelle $||f||_{C^k}=\sum_{j=0}^k ||D^j f||_{\infty} $. L'opérateur de dérivation 
\begin{equation*}
...
\begin{exemple}
    ...
    \end{equation*}
    Posons $I:f\mapsto \int_{0}^1 yf(y)dy$ soit $g\in \im F$, c'est-à-dire qu'il existe $f$ tel que 
    \begin{equation*}
        f(x)=g(x)+3xI(f)
    \end{equation*}
    on a alors 
    ...
    \end{equation*}
    et donc $g$ est dans le noyau de la forme linéaire $I$. Bien sûr, si $g$ est dans le noyau de $I$ alors c'est un point fixe de $F$ et donc $\im F= \ker I$, le noyau d'une forme linéaire continue non nulle étant de codimension $1$, la fonction est de Fredholm d'indice nul.
\end{exemple}
...
On rappelle également  que la norme habituelle sur le quotient est donnée par 
    ...
    \end{equation*}
    Il est bien connu que le quotient d'un espace de Banach par un fermé est un espace de Banach (voir par exemple le cours de Céline Esser \cite{EsserFONCT}). Nous montrons quelques propriétés élémentaires des opérateurs de Fredholm, les résultats suivants sont adaptés de \cite{abramovich2002invitation}
\begin{lemme}
...
\end{lemme}
\begin{preuve}
    Si $\im D$ est fermé, il suffit de prendre $Y_0=\{0\}$. Maintenant, supposons avoir $Y_0$ vérifiant les conditions de l'énoncé et posons $G=\im D\oplus Y_0$. Considérons $G/Y_0$ muni de la norme quotient habituelle et la fonction 
    \begin{equation*}
    ...
    $J$ est un isomorphisme d'espace de Banach et donc $\exists c>0$ tel que $c||y||_d\leq ||J(y)||_{G/Y_0}$ pour tous $y\in \im D $ on a alors 
    \begin{equation*}
...
\begin{lemme}
Si $X_0$ est un sous-espace de dimension finie de $X$ alors il est \coldef{complémenté},i.e., il existe un sous-espace fermé $X_1$ tel que $X_1\oplus X_0=X$
\end{lemme}
de ce qui précède, on déduit le résultat suivant
\begin{proposition}\label{spliting}
...
\begin{definition}
    Si $G_0,G_1,\cdots, G_n$ sont des groupes (ou espaces vectoriels, modules, etc...)\footnote{Plus généralement, des objets d'une catégorie semi-abélienne, voir \coldef{\href{https://ncatlab.org/nlab/show/exact+sequence}{nlab}} pour plus d'informations} alors la suite 
    \begin{equation*}
...
\begin{definition}
    Soient $X,Y$ des espaces de Banach, $F:X\rightarrow Y$ de Fredholm en $0$ et $\operatorname*{Ind}(DF(0))=0$. Considérons alors une base $B=x_1,\cdots,x_n$ de $\ker DF(0)$ et  $B^{*}=y^{*}_1,\cdots,y^{*}_n$ une base de $\ker{ DF(0)}^{*}$. Le théorème de Hahn-Banach nous assure l’existence d'une complétion bi-orthogonale, i.e., 
    \begin{equation*}
...
Nous montrons maintenant quelques propriétés utiles des opérateurs de Fredholm. 
 Rappelons qu'une fonction continue entre deux espaces topologiques $f:E\rightarrow F$ est \textcolor{color-def}{propre} si la pré-image de tout compact est compact et \textcolor{color-def}{ localement propre} si pour tout $x\in E$ il existe un voisinage $V'$ de $x$ et un voisinage $U$ de $f(x)$ tel que $f|_{V'}:V\rightarrow U$ est propre.
 \begin{remarque}
    Bien que nous le montrions pas ici, il existe une large  classe d'espaces (appelés $k$-espaces) pour lesquels les fonctions propres sont fermées, et les espaces de Banach en font partie. Et il est vrai, en toute généralité, que si une fonction est fermée et que les images inverses de points sont compactes, alors elle est propre. Le lecteur intéressé peut consulter les cours de Lee et Ryszard, par exemple. \cite{Lee2011TopologicalManifolds,Engelking1989GeneralTopology}
 \end{remarque}

Les résultats suivants sont dus à  \cite{Smale1965Sard}
\begin{proposition}\label{propre-fred}
    Si $F:X\rightarrow Y$ est de Fredholm, alors $F$ est localement propre
\end{proposition}
\begin{preuve}
Soit $x_0\in X$ et $U\ni x_0$ un voisinage. On pose $R=\im DF(x_0)$ et $N=\ker DF(x_0)$
Comme $DF(x_0)$ est de Fredholm, on a, via \ref{spliting}, $X\cong X_0\times N $ et $Y\cong Y_0\times R$. On pose $x_0=(u_0,v_0)$, remarquez alors que $D_1F(u_0,v_0): X_0\rightarrow Y$ est injective et d'image fermée. Considérez $\pi: Y\rightarrow R$ la projection associée à la décomposition ci-dessus, on a que $D_1\pi\circ F$ est un isomorphisme d'espace de Banach et on pose
\begin{equation*}
    H: X_0\times N\rightarrow R\times N, (u,v)\mapsto (\pi F(u,v),v).
\end{equation*}
On remarque alors que
\begin{equation*}
    DH(u_0,v_0)=\begin{pmatrix}
        \pi\circ D_1F(u_0,v_0) && 0\\
        0 && I\\
    \end{pmatrix}
\end{equation*}
C'est un isomorphisme d'espace de Banach, on peut alors appliquer le théorème des fonctions inverses, qui nous fournit des voisinages $U_1\times V_1\ni (u_0,v_0) $ et $W\ni H(u_0,v_0)$ tels que $H$ est un $C^1$ de $U_1\times V_1$ vers $W$. $W$, on sait que $H^{-1}$ est de la forme $H^{-1}(r,v)=(\varphi(r,v),v)$. On choisit alors un rectangle fermé $A=\bar{B}_{R}(\pi F(x_0),\delta)\times \bar{B}_{N}(v_0,\delta) \subseteq W$. Étant donné que $N$ est de dimension finie, on peut supposer $\bar{B}_{N}(v_0,\delta)$ compact. On pose enfin $U_0=H^{-1}(A)$. On veut alors montrer que $F|_{U_0}$ est propre. Considérons alors un compact $K\subseteq  Y$ et $(x_n)_{n}$ une suite de $F|_{U_0}^{-1}(K)$ et  $(r_n,v_n)=H(x_n)$. Par définition $r_n=\pi F(x_n)$, et donc, du fait que $\pi$ est continu, $\pi(K)$ est compact et on peut alors supposer $r_n\to r$ et $\bar{B}_{N}(v_0,\delta)$  étant compact,  $v_n\to v$. Par continuité de $H^{-1}$, on a que $x_n=H^{-1}(r_n,v_n)\to H^{-1}(r,v)$.  Et on conclut par la continuité de $F$.
\end{preuve}
...
\begin{theobox}{Morse-Sard}{}\label{morse-sard}
    Si $U$ est un ouvert de $\R^p$ et $F\in C^k(U,\R^q)$ où $k>\max(p-q,0)$ alors l'ensemble des valeurs singulières a une mesure nulle 
\end{theobox}
...
\end{definition}
\begin{theobox}{Morse-Sard-Smale}{}
...
\end{preuve}
\begin{corollaire}
    Dans les hypothèses du théorème précédent,
...
$(x_0,\lambda_0)$, le théorème \ref{Atkinson} permet d'affirmer que l'indice de $F(\cdot,\lambda)$ y sera constant, mais qu'en est-il de la fonction totale $F:X\times \Lambda\rightarrow Y$ ?
Des calculs simples permettent d'établir que
\begin{equation*}
   \operatorname*{Ind}(DF(x_0,\lambda_0))
   =\operatorname*{Ind}(D_1F)+ \dim(\Lambda)
\end{equation*}
C'est pour cela qu'en pratique, on se concentrera surtout sur le cas où $ \operatorname*{Ind}F(\cdot,\lambda)\geq -\dim \Lambda$
...
bifurcation sont "rares", et que la "plupart du 
temps"(de façon générique voir \cite{Hirsch1976DifferentialTopology}), il s'agit d'une variété. Notez cependant qu'un ensemble maigre peut avoir des points d'accumulation ($\mathbb{Q}$, par exemple).
\begin{exemple}
    On peut construire une fonction $F\in C^{\infty}(\R^2,\R)$ pour laquelle l'ensemble des $y\in \R$ tels que $F^{-1}(y)$ n'est pas une variété est $\mathbb{Q}$. Considérons une énumération des rationnels $(r_n)_{n\in\mathbb{\mathbb{Z}}}$ et considérons la suite
    \begin{equation*}
        a_{n}=\begin{cases}
            r_{n/2}\text{ si $n$ est pair}
            \\ \min (r_{(n-1)/2},r_{(n-1)/2+1})-1 \text{ sinon}
        \end{cases}
    \end{equation*}
...
soit "la plupart du temps" une variété de dimension $\operatorname*{Ind}(F)$. Ça n'est bien sûr pas vrai en général, même avec des fonctions non nulles: 
...
Dans cette dernière sous-section, nous motivons l'étude des opérateurs de Fredholm en présentant le cadre le plus naturel dans lequel ils apparaissent. Nous nous contenterons de donner les définitions et les énoncés importants, le lecteur intéressé par les détails pourra consulter \cite{berger1977nonlinearity}
...
\begin{definition}
    ...
    $P(x,D)=\sum_{|\alpha|\leq m} c_{\alpha}(x) D^{\alpha}$ est un polynôme de degré au plus $m$ en $D_1,\cdots,D_n$, $l\geq m\geq 1$ et $c_\alpha\in C^{l-m}(W,\R)$ avec $W$ un ouvert contenant $\bar{U}$ alors un opérateur de la forme 
...
\begin{definition}
    ...
    {elliptique } en $x_0$ si le polynôme $P(x_0,\xi)_m$ est 
    de signe constant sur $\R^n\setminus\left\{0\right\}$. Notez que cela impose à $m$ d'être pair. Il est elliptique sur $U$ s'il l'est en tout point de $U$ et 
    s'il existe une constante $c>0$ telle que 
    pour tout $x\in U$,$\xi\in\R^n$ on ait $P_m(x,\xi)\geq 
    c |\xi|^m$ alors il est  \coldef{uniformément 
    elliptique }
\end{definition}
Le contexte fonctionnel le plus simple est celui des espaces de Hölder: on rappelle que fonction est $\alpha$-hölderiennes ($0<\alpha<1$)sur $\bar{U}$ si
\begin{equation*}
    ||f||_{C^{\alpha}}=||f||_{\infty}+\sup_{x\neq y}\frac{|f(x)-f(y)|}{|x-y|^{\alpha}}<\infty
\end{equation*}
On note $C^{\alpha}(\bar{U})$ l'ensemble des fonctions $\alpha$-hölderienne sur $\bar{U}$ et $C^{k+\alpha}(\bar{U})$ l'ensemble des fonctions dont toutes les dérivées d'ordre $\leq k$ sont $\alpha$-hölderiennes.  Il est bien connu que $(C^\alpha(\bar{U}),||\cdot||_{C^{\alpha}})$ est un espace de Banach et en posant
\begin{equation*}
...
\begin{proposition}
    Soit $U$ une partie ouverte et bornée de $\R^n$ ayant un bord $\partial U$ lisse. Soit $E$ un opérateur elliptique d'ordre $2k$ sur $\bar{U}$ à coefficients lisses (ou au moins $C^{l+\alpha}$)
    \begin{equation*}
        X=\left\{ f\in C^{2k+l+\alpha}(\bar{U}):\forall |\beta|\leq k-1, D^{\beta}f|_{\partial U}=0\right\}\text{ et } Y=C^{l+\alpha}(\bar{U})
    \end{equation*}
    Alors $E: X\rightarrow Y$ est un opérateur de Fredholm d'indice 0
\end{proposition}
\begin{preuve}
    voir \cite{berger1977nonlinearity}
\end{preuve}
Ce type d'opérateurs est très courant en physique, par exemple:
\begin{exemple}
    Hamiltonien de Schrödinger magnétique
    \begin{equation*}
        H_{A,V}=\frac{1}{2m}(-ih\nabla - qA(x))^2+V(x)
    \end{equation*}
