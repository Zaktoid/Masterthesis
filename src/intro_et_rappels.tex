...
$F\in C^0(U,Y)$ est \coldef{dérivable} en $x_0\in U$ s'il existe $h(x)=f(x_0)+u(x-x_0)$ tangent en $x_0$, ce $u$ est alors \coldef{la dérivée de $F$ en $x_0$} qu'on notera $dF(x_0)$. Naturellement, $F$ est dérivable sur $U$ si elle est dérivable en chaque point
\end{definition}
...
\end{definition}
Nous verrons plus tard que si $f$ est de Fredholm en un point, alors elle l'est sur un voisinage connexe $V$ et son indice y est constant.
...
\begin{preuve}
    ...
    précédent, il existe un voisinage $W=U_1\times V_1$ sur lequel la propriété reste vraie et on peut supposer $V_1$ compact. quitte à réduire $U_1$ et $V_1$, on peut supposer pouvoir appliquer le théorème des fonctions implicites qui nous permet alors d'affirmer que $F(\cdot,v_1)|_{U_1}$ est un homéomorphisme sur son image si $v_1\in V_1$. Maintenant, soit $K$ un compact de $Y$, nous devons montrer que $A=F^{-1}(K)\cap W$ est compact. Il suffit de montrer qu'il est séquentiellement compact, soit donc $(u_i,v_i)_{i\in\mathbb{N}}$ une suite de $A$. $F(u_i,v_i)$ est une suite de $K$, qu'on peut supposer converger vers $y\in K$ , et $V_1$ étant compact, on peut supposer $v_i\to v$. De plus, en appliquant l'inégalité des accroissements finis, on peut même supposer $v_i=v$. $F(\cdot,v)|_{D_1}$ étant un homéomorphisme, on conclut.
\end{preuve}
...
\end{theobox}
\begin{preuve}
    Les espaces de de Banach séparables étant à 
    ...
    et 
    \begin{equation*}
        \varphi : \times \ker DF(x_0)\rightarrow  Y_0, v\mapsto \pi (F(u_0,v))
    \end{equation*}
