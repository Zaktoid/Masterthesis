...
\begin{equation*}
    ...
    \end{pmatrix}
\end{equation*}
C'est un isomorphisme d'espace de Banach on on peut alors appliquer le théorème des fonctions inverses, qui nous fourni des voisinages $U_1\times V_1\ni (u_0,v_0) $ et $W\ni H(u_0,v_0)$ tel que $H$ est un $C^1$ de $U_1\times V_1$ ver $W$. $W$, on sait que $H^{-1}$ est de la forme $H^{-1}(r,v)=(\varphi(r,v),v)$. On choisi alors un rectangle fermé $A=\bar{B}_{R}(\pi F(x_0),\delta)\times \bar{B}_{N}(v_0,\delta) \subseteq W$. Étant donné que $N$ est de dimension finie, on peut supposer $\bar{B}_{N}(v_0,\delta)$ compact. On pose enfin $U_0=H^{-1}(A)$. On veut alors montrer que $F|_{U_0}$ est propre. Considérons alors $K\subseteq  Y$ et $(x_n)_{n}$ une suite de $F|_{U_0}^{-1}(K)$ et  $(r_n,v_n)=H(x_n)$. Par définition $r_n=\pi F(x_n)$, et donc, du fait que $\pi$ est continu, $\pi(K)$ est compact et on peut alors supposer $r_n\to r$ et $v_n\to v$. Par continuité de $H^-1$, on a que $x_n=H^{-1}(r_n,v_n)\to H^{-1}(r,v)$. Ce qui permet de conclure
\end{preuve}
...
\begin{exemple}
    ...
    \begin{equation*}
        a_{n}=\begin{cases}
            r_{k/2}\text{ si $k$ est pair}
            \\ \min (r_{(k-1)/2},r_{(k-1)/2+1})-1 \text{ sinon}
        \end{cases}
    \end{equation*}
    ...
    \end{equation*}
    Maintenant si $y=r_k$, alors $a_{2k-1}<a_{2k}$ et $a_{2k+1}<a_{2k}$ et $2k$ alors un maximum local strict et on a donc localement
    \begin{equation*}
        F(x,\lambda)=r_k\Leftrightarrow \lambda^2=r_k-g(x)
    \end{equation*}
\end{exemple}
On aimerait 
