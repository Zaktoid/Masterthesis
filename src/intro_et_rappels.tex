...
\begin{equation*}
    ...
    \end{pmatrix}
\end{equation*}
C'est un isomorphisme d'espace de Banach on on peut alors appliquer le théorème des fonctions inverses, qui nous fourni un voisinage $U_1\times V_1$ tel que $F(\cdot,v)|_{U_1}$ est un difféomorphisme sur son image. $N$ étant de dimension finie, on peut supposer $V_1$ compact. Soit maintenant $K$ un compact de $Y$, on doit montrer que $L=F^{-1}(K)\cap (U_1\times V_1)$ est compact. Soit $(u_i,v_i)$ une suite de $L$, du fait que $V_1$ est compact, on peut supposer que $v_i\to v$. Mais étant donné que $K$ est compact, on peut supposer $F(u_i,v)\to F(x)$. L'inégalité des accroissements finis nous permet enfin de supposer que $v_i=v$. Mais étant donné que $F(\cdot,v)|_{U_1}$ est un homéomorphisme, on a que $u_i\to u$
\end{preuve}
