...
\begin{corollaire}
    ...
    Si $DF(x_0)$ est un isomorphisme alors il existe $U'\subseteq U$u n voisinage ouvert de $x_0$ et $V'$ un voisinage de $y_0$ tels que
...
Nous montrons maintenant quelques propriétés utiles des opérateurs de Fredholm. 
 ...
 \begin{remarque}
    Bien qu'on ne le montrera pas ici, il existe une large  classe d'espaces (appelés $k$-espaces) pour lesquels les fonctions propres sont fermées, et les espace de Banach en font partie. Et il est vrai, en toute généralité, que si une fonction est fermée et que les images inverses de points sont compactes, alors elle est propre. Le lecteur intéressé peut consulter les cours de Lee et Ryszard, par exemple. \cite{Lee2011TopologicalManifolds,Engelking1989GeneralTopology}
 \end{remarque}

On rappelle deux propriétés basiques sur les opérateurs d'espace de Banach, pour une preuve, voir par exemple le cours de W. Rudin (\cite{rudin1991functional})
\begin{lemme}
...
\end{proposition}
\begin{preuve}
Soit $x_0\in X$ et $U\ni x_0$ un voisinage. On on pose $R=\im DF(x_0)$ et $N=\ker DF(x_0)$
...
\begin{equation*}
    ...
    \end{pmatrix}
\end{equation*}
C'est un isomorphisme d'espace de Banach on on peut alors appliquer le théorème des fonctions inverses, qui nous fourni des voisinages $U_1\times V_1\ni (u_0,v_0) $ et $W\ni H(u_0,v_0)$ tel que $H$ est un $C^1$ de $U_1\times V_1$ ver $W$. $W$, on sait que $H^{-1}$ est de la forme $H^{-1}(r,v)=(\varphi(r,v),v)$. On choisi alors un rectangle fermé $A=\bar{B}_{R}(\pi F(x_0),\delta)\times \bar{B}_{N}(v_0,\delta) \subseteq W$. Étant donné que $N$ est de dimension finie, on peut supposer $\bar{B}_{N}(v_0,\delta)$ compact. On pose enfin $U_0=H^{-1}(A)$. On veut alors montrer que $F|_{U_0}$ est propre. Considérons alors un compact $K\subseteq  Y$ et $(x_n)_{n}$ une suite de $F|_{U_0}^{-1}(K)$ et  $(r_n,v_n)=H(x_n)$. Par définition $r_n=\pi F(x_n)$, et donc, du fait que $\pi$ est continu, $\pi(K)$ est compact et on peut alors supposer $r_n\to r$ et $\bar{B}_{N}(v_0,\delta)$  étant compact,  $v_n\to v$. Par continuité de $H^{-1}$, on a que $x_n=H^{-1}(r_n,v_n)\to H^{-1}(r,v)$.  Et conclut par la continuité de $F$.
\end{preuve}
...
\end{preuve}
\begin{corollaire}
    Dans les hypothèse du théorème précédent,
...
\begin{exemple}
    On peut construire une fonction $F\in C^{\infty}(\R^2,\R)$ pour laquelle l'ensemble de $y\in \R$ tel que $F^{-1}(y)$ n'est pas une variété est $\mathbb{Q}$. Considérons une énumération des rationnels $(r_n)_{n\in\mathbb{\mathbb{Z}}}$ et considérons la suite
    \begin{equation*}
        a_{n}=\begin{cases}
            r_{k/2}\text{ si $n$ est pair}
    ...
    \end{equation*}
    Maintenant si $y=r_n$, alors $a_{2n-1}<a_{2n}$ et $a_{2n+1}<a_{2n}$ et $2k$ alors un maximum local strict et on a donc localement
    \begin{equation*}
...
Dans cette dernière sous-section, nous motivons l'étude des opérateurs de Fredholm en présentant le cadre le plus naturel dans lequel il apparaîssent. Nous nous contenterons de donner les définitions et les énoncés importants, le lecteur intéressé par les détails pourra consulter \cite{berger1977nonlinearity}
...
\begin{definition}
    ...
    de signe constant sur $\R^n\setminus\left\{0\right\}$. Notez que cela impose à $m$ d'être pair. Il est elliptique sur $U$ si il l'est en tout point de $U$ et 
    s'il existe un constante $c>0$ telle que 
    pour tout $x\in U$, on aie $P_m(x,\xi)\geq 
    c |\xi|^m$ alors il est  \coldef{uniformément 
    elliptique }
\end{definition}
Le contexte fonctionnel le plus simple est celui des espaces de Hölder: on rappelle que fonction est $\alpha$-hölderienne sur $\bar{U}$ si
\begin{equation*}
    ||f||_{C^{\alpha}}=||f||_{\infty}+\sup_{x\neq y}\frac{|f(x)-f(y)|}{|x-y|^{\alpha}}<\infty
\end{equation*}
On note $C^{\alpha}(\bar{U})$ l'ensemble des fonctions $\alpha$-hölderienne sur $\bar{U}$ et $C^{k+\alpha}(\bar{U})$ l'ensemble des fonctions dont toutes les dérivées d'ordre $\leq k$ sont $\alpha$-hölderienne.  Il est bien connu que $(C^\alpha(\bar{U}),||\cdot||_{C^{\alpha}})$ est un espace de Banach et en posant
\begin{equation*}
...
\begin{proposition}
    ...
    \begin{equation*}
        X=\left\{ f\in C^{2k+l+\alpha}(\bar{U}):\forall |\alpha|\leq k-1, D^{\alpha}f|_{\partial U}=0\right\}\text{ et } Y=C^{l+\alpha}(\bar{U})
    \end{equation*}
...
\begin{exemple}
    Si $g$ est une métrique riemannienne lisse, le Laplacian $\Delta_g$ est elliptique. En particulier, l'opérateur de Klein-Gordon euclidien (utilisé en QFT relativiste) défini par
    \begin{equation*}
        P=-\Delta_g+m^2+cR_g
    \end{equation*}
