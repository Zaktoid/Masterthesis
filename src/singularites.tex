...
\begin{definition}
    ...
    $\mathcal{G}_{un}$ cette relation d'équivalence
\end{definition}
...
\begin{theobox}{Théorème du déploiment universel}{}
Soit $f$ un germe, un déploiement à $k$ paramètres $G$  est  universel si et seulement si
\begin{equation*}
    \mathcal{E}^m_{p+q}=T_f\mathcal{B}+ \rangle \partial_{\alpha_1}G(\cdot,\cdot,0),\cdots, \partial_{\alpha_k}G(\cdot,\cdot,0)\langle_{\R}
\end{equation*}
...
l'espace tangent $T_f\mathcal{B}$, 
...
Pour la preuve de la suffisance, on se base sur \cite{golubitsky1979theory}. On prouve alors quelques résultats préliminaires 
\begin{proposition}
    Soit $N$ un module finiment 
    généré sur 
    $\mathcal{E}_{p+q+k}$. Si on pose $N_0=N/\mathfrak{m}_{k}N$ alors
    \begin{equation*}
    \rangle n_1,\cdots, n_l \langle_{\mathcal{E}_k}=N
    \text{ si et seulement si } \rangle [n_1],\cdots [n_l]\langle_{\R}=N_0
    \end{equation*}
\end{proposition}
\begin{preuve}
    \todo{écrire
...
\end{preuve}
\begin{proposition}
    Soit $G\in\mathcal{E}^m_{p+q+1+k}$ un déploiement d'un germe $f\in\mathcal{E}_{p+q+1}^m$. Supposons que $G$ soit de codimension finie. Soient de plus $q_1$
\end{proposition}
\newpage
\end{document}