...
(\lambda_0)\}$.\\ 
Si on a un germe  
$g(x,\lambda)$ un peut se demander quelles sont les perturbations qui conservent sa structure, ou plus précisemment, qui ne la font pas changer de classe d'équivalence  pour la relation \ref{GolubEQ}. Soit $p(x,\lambda)$ un germe  et considérons $G\in\mathcal{E}_{p+q+1}^m$ telle que
...
\begin{exemple}
    ...
    \begin{equation*}
        ...
        \mid A\in\mathcal{E}(0,\R^{p+k},\R^m_m), B\in\mathcal{E}^p_{p+k}\mathcal{M}_{p+k} \right\}
    \end{equation*}
    On a alors
    \begin{equation*}
        T_f \mathcal{RB}=\rangle f_1,\cdots, f_m\langle_{\mathcal{E}_{p+k}}\cdot\mathcal{E}^m_{p+k}+\rangle  \partial_{x_1}f,\cdots,\partial_{x_p}f\langle_{\mathcal{E}_{p+k}}\cdot \mathcal{M}_{p+k} .
    \end{equation*}
...
\subsubsection{Etude de l'esapce tangent}
...
On commence par une charactérisation de l'appartenance à l'espace tangent. Posons $\mathcal{E}_{p+k}^{m\times n}=\mathcal{E}(0,\R^{p+k},\R^m_n)$
...
\begin{preuve}
    ...
    \begin{equation*}
        g=a(x,\lambda)f(x,\lambda)+ \partial_x f b(x,\lambda)\text{ avec }a\in\mathcal{E}_{p+k}^{m\times m}\text{ et }b\in\mathcal{E}^p_{p+k}\mathcal{M}_{p+k}
    \end{equation*}
...
\begin{equation}\label{taylor}
    ...
    {\alpha}\text{ et } f = J^kf\pmod {\mathcal{M}_n^{k+1}}
\end{equation}
...
\begin{exemple}
    ...
    \begin{equation*}
        E=\left\{f\in\mathcal{E}_2^1: f(0)=\partial_x f(0)=\partial_\lambda f(0)=\partial^2_{\lambda,\lambda} f(0)=\partial^2_{x,x} f(0)=0\right\}
    \end{equation*}
\end{exemple}
...
\begin{exemple}
    ...
    charactériser le tangent avec des conditions sur un nombre fini de dérivées de $F$, contrairement aux 2 autres.
\end{exemple}

\begin{remarque}
    Nous remarquons déjà une différence nette entre la géométrie de la variété $\mathcal{RB}\cdot f$ et ce qui se passe en dimension finie. Prenons un élément du tangent de la bifurcation trident, $v=x^3-\lambda^2$, par exemple. Dans la suite, on prouvera (en autre) que $f+\epsilon v\in\mathcal{RB}\cdot f$  si $\epsilon>-1$, et de même, pour tout vecteur du tangent, on va avoir $f+\epsilon v$ qui reste dans l'orbite si $v$ assez petit.
\end{remarque}
...
On remarque alors que les fonctions "plus 
dégénérées" on un espace tangent plus petit, 
...
\end{equation*}
donc $\codim(x^2+\lambda)=2$ et $\codim(x^3-\lambda)$
...
comment obtenir une charactérisation en qui ne 
fait intervenir que les valeurs des dérivées 
partielles?

\begin{lemme}
    Si $f\in\mathcal{E}^m_{p+q}$ et pour un certain $k\geq 0$, $\mathcal{M}^{k+1}_{p+q}\cdot \mathcal{E}^m_{p+q}\subseteq T_f\mathcal{RB}$. Alors $g\in T_f\mathcal{RB} $ si et seulement si il exste des germes de matrices à éléments polynômiaux de  degré au plus $k$ telles que
...
\begin{preuve}
    ...
    \begin{equation*}
        ...
        \right)\in\mathcal{M}^{k+1}_{p+q}\cdot\mathcal{E}^m_{p+q}\subseteq T_f\mathcal{RB}.         
    \end{equation*}
    Et $g$ s'écrit alors comme la somme de deux éléments du tangent, ce qui suffit.
\end{preuve}

Ainsi, si l'espace tangent est "suffisemment gros", on a une procédure explicite pour charactériser ses éléments en terme de dérivées. Evaluons la difficulté de cette procédure en fonction de cette quantité $k$. Il faut pour commencer compter le nombre de $\alpha_1,\cdots,\alpha_{p+q}\in\mathbb{N}$ tels que
\begin{equation*}
...
dimensions.\\
Cependant, il faut noter que $E_1,E_2$ représente le maximum d'équations et de variables, en pratique, il est souvent bien plus petit, mais ça ne sauvera pas cette approche si le nombre de paramètres et les dimensions deviennetnt trop élevés.
\begin{exemple}
    \todo{Exemple à 2 paramètres avec calculs}
\end{exemple}
...
\begin{proposition}
    ...
    $\mathcal{E}^m_{p+q}$. Il existe un entier $k$ tel que $\mathcal{M}_{p+q}^k\cdot\mathcal{E}^m_{p+q}\subseteq M$ si et seulement si $M$ est de codimension finie
\end{proposition}
