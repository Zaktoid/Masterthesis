...
$g(x,\lambda)$ un peut se demander quelles sont les perturbations qui conservent sa structure, ou plus précisemment, qui ne la font pas changer de classe d'équivalence  pour la relation \ref{GolubEQ}. Soit $p(x,\lambda)$ une germe, considérons la germe $h(t,x,\lambda)=(g(x,\lambda)+tp(x,\lambda))$ 
...
\end{preuve}
La relation d'équivalence \ref{GolubEQ} peut aussi s'écrire comme une action de groupe: Si on pose $\mathcal{C}$ comme étant l'ensemble des germes de 
 On peut alors reformuler le problème de la façon suivante: $f\in\mathcal{E}(0,\R^n,\R^m)$  est $k$-$E$-déterminée si $\operatorname{orb}_G(f)$ contient les germes ayant le même jet d'ordre $k$ que $f$. On veut un équivalent de ce  résultat de théorie des groupes de Lie 
\begin{theobox}{}{}
...
entend par $\mathcal{V}(f)$ la germe en $0$ 
...
\begin{exemple}
    ...
    est simplement une germe lisse de la forme
    \begin{equation*}
        (x,\lambda,u)\mapsto f(H(x,\lambda,u),
        \lambda)\cdot S(x,\lambda,u)
    \end{equation*}
    ...
    On a alors
    \begin{equation*}
        T_f \mathcal{RB}=\rangle f_1,\cdots, f_m\langle_{\mathcal{E}_{p+k}}\cdot\mathcal{E}^m_{p+k}+\rangle  \partial_{x_1}f,\cdots,\partial_{x_p}f\langle_{\mathcal{E}_{p+k}}
    \end{equation*}
