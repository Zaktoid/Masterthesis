...
De façon remarquable, en dimension finie, il y'a qu'une seule fonction qui possède ces proriétés ! On peut donc parler "du" degré. Nous donnons directement la définition, la construction (qui est faite via le lemme de Sard et des résultats d'approximation)
...
L'espoir de généraliser entièrement la notion de degré aux fonction définie sur $U$ un ouvert d'un espace de Banach $X$ et à valeur dans $X$ s'effondre à l'instant où on constate qu'on peut montrer le théorème du point fixe de Brouwer \textit{uniquement} avec ce dernier, mais qu'il n'est pas vrai en dimension infinie ! \\
\todo{Construction du degré en dimension infinie et propriétés de base}
