...
\subsection{Notions de calcul différentiel dans des Espaces de Banachs}
...
\subsubsection{Opérteurs elliptiques}

Nous rappelons qu'un multi-indice $\alpha$ de taille $n$ est simplement un tuple de nombre $(\alpha_1,\cdots,\alpha_n)$ et qu'on a $D^{\alpha}=D^{\alpha_1}_1\cdots D^{\alpha_n}_n$, on pose aussi $|\alpha|=\sum_{i=1}^n \alpha_i$ et $\alpha !=\alpha_1\cdots\alpha_n$.
\begin{definition}
    Si $U$ est un ouvert de $\R^n$, un \textit{opérateur diférentiel partiel} est un operateur de la forme 
    \begin{equation*}
        F=\sum_{|\alpha|\leq m} a_{\alpha}(x)D^{\alpha}
    \end{equation*}
    où les $a_{\alpha}$ sont des fonctions de $U$ vers $\R$ (ou $\mathbb{C}$)
    Si il existe $a_\alpha$ non-identiquement nul avec $\alpha=n$ alors l'opérateur
\end{definition}
...
\subsection{Approche via la thoérie des singularités}\label{SINGsection}
...
De façon remarquable, en dimension finie, il y'a qu'une seule fonction qui possède ces proriétés ! On peut donc parler "du" degré.

\begin{definition}
    Si $U\subseteq \R^n$ est ouvert et bornée, si $f\in C^0(\bar{U})\cap C^1(U)$ et $y\in \R^n\setminus f(\partial U \cup S_f(U))$ alors on pose 
    \begin{equation*}
        \operatorname{deg}(f,U,y)=\sum_{x\in f^{-1}(y)} \operatorname*{sign} \det Df(x)
    \end{equation*}
\end{definition}
On peut montrer que cette fonction remplit bien les 3 conditions et que c'est la seule. Ensuite, via le théorème de Morse-Sard et quelques techniques d'approximation classiques, on peut généraliser cette fonction à $C^0(\bar{U})$, voir \cite{KlausNonLin85} pour les détails.
