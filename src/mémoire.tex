...
triviale}, i.e,  si $(x_0,
\lambda_0)\in X\times 
\Lambda$ dans quelles conditions est-ce que tout voisinage de $(x_0,\lambda_0)$ contient des solutions \textit{non triviales}, c'est-à-dire des zéros  de $F$ ne se trouvant pas sur la branche triviale.\\
 La première partie de ce travail établit les notions prérequises et a pour but de le rendre le plus \textit{self contained} que possible. La seconde partie introduit la \textit{réduction} de Lyapunov-Schmidt, qui,  dans des conditions que nous allons préciser, permet de se ramener à un problème en dimension finie. Ensuite, nous introduirons l'approche par \textit{ la théorie des singularités} initiée par Golubitsky dans \cite{golubitsky1985sgb1} et reformulée et enrichie par Montaldi dans \cite{montaldi2021sbc}. Cette approche nous permettra de \textit{classifier} les bifurcations, i.e, étant donné une fonction $F(x,\lambda)$ et un point de son domaine $(x_0,\lambda_0)$ autour duquel on veut étudier le comportement de $F$, il peut être utile de se poser la question suivante: si je m'intéresse à une propriété quelconque $P$, quelles sont les  transformations $T$ qui laissent $P$ invariante ? Le but étant bien sûr de se ramener dans un cas \textit{plus simple} sans perdre $P$. Cela nous ramène naturellement à étudier le travail de Wall dans \cite{Wall1981FiniteDeterminacy}. Nous verrons alors ces transformations comme des \textit{actions de groupes} et étudierons leurs orbites.\\
 La troisième partie présente des techniques d'un tout autre type: nous utilisons des outils de topologie et topologie algébrique pour détecter \textit{globalement } les bifurcations (depuis une branche triviale). D'abord, nous exposons les approches classiques par théorie du degré topologique, en nous basant sur \cite{Feckan2008Topological} pour ensuite introduire des développements plus modernes: le degré \textit{orienté} de Furi \cite{BenevieriFuri2001} ainsi que le fibré virtuel d'index de Pejsachowicz \cite{Pejsachowicz2011Index}\\
 La dernière partie, quant à elle, constituera une introduction aux bifurcations dynamiques. Si on se replace dans le contexte d'un système dynamique (donc l'étude d'une équation d'évolution comme  \ref{equanewton}), on s'intéresse à comment les propriétés purement dynamiques changent quand $\lambda$ varie, dans ce travail, nous exposerons uniquement la plus fameuse bifurcation dynamique : la bifurcation de Hopf. Intuitivement, elle correspond à un passage d'un régime présentant des \textit{ points d'attraction ou de répulsion} à un régime présentant des \textit{orbites périodiques attractives ou répulsives}, nous en profiterons également pour introduire quelques notions de stabilité. Nous n'explorerons pas la dynamique plus loin, mais notez qu'on peut également considérer des bifurcations correspondant à un passage d'un système non chaotique à un système chaotique (voir par exemple la fameuse "logistic map").
...
\subsubsection{Notions de calcul différentiel dans des Espaces de Banach}

dans cette section nous rappelons brièvement les résultats classiques du calcul différentiel dans des espaces de Banach. Le but étant de pouvoir présenter le théorème des fonctions implicites, un des outils principaux de la théorie de la bifurcation, nous nous basons principalement sur \cite{dieudonne1969fma8} et \cite{coleman2012calculus}. Dans la suite, les lettres $X,Y,\Lambda,Z$ font référence à des espaces de Banach
...
$F\in C^0(A,Y)$ est \textit{dérivable} en $x_0\in U$ s'il existe $h(x)=f(x_0)+u(x-x_0)$ tangent en $x_0$, ce $u$ est alors \textit{la dérivée de $F$ en $x_0$} qu'on notera $dF(x_0)$. Naturellement, $F$ est dérivable sur $A$ si elle est dérivable en chaque point
\end{definition}
...
\begin{definition}
    ...
    est dérivable, on note  $d_{i}f(a)$ sa dérivée. Si $1\leq k<j \leq n$, alors on considère  $\tilde{f}$ comme étant $f$ mais définie sur $(\prod_{i=1}^{k-1} X_i)\times (\prod_{i=k}^{j} X_i)\times (\prod_{i=j+1}^{n} X_i)$ et on pose dans ce cas $d_{k,\cdots,j }f=d_2 \tilde{f}$ 
\end{definition}
\begin{proposition}
...
\begin{definition}
    ...
    $\mathcal{F}_n(X,Y)$  l'ensemble des opérateurs de Fredholm d'indice $n$ et $\mathcal{F}(X,Y)$ l'ensemble des opérateurs de Fredholm. Enfin, on dit qu'une fonction $F: X\rightarrow Y$ est \textit{de Fredholm en } $x\in X$ si elle est dérivable en $x$ et si $df(x)$ est de Fredholm. Une fonction est de Fredholm sur un ouvert $U$ de $X$ si elle est $C^1$ sur $U$ et si sa dérivée est de Fredholm sur $U$
\end{definition}
...
\end{preuve}
Le résultat suivant est un corollaire direct du théorème de Hahn-Banach, on l'admet.
\begin{lemme}
...
On définit alors \textit{l'opérateur de Schmidt associé à}  $(B,B^{*})$
...
\begin{definition}
    Si $U$ est un ouvert de $\R^n$, et $l\geq m\geq 1$ un \textit{opérateur différentiel partiel} est un opérateur de la forme 
...
\begin{definition}
    ...
    \end{equation*}
    et si les coefficients principaux sont uniformément bornés dans $U$ alors l’opérateur $E$ est \textit{Uniformément fortement elliptique }
\end{definition}
...
On peut déjà présenter un cas très simple, la "bifurcation" \textit{fold} (repliement), on met des guillemets car tous les auteurs ne la considèrent pas comme une "vraie" bifurcation, cela deviendra clair dans la suite. 
...
\begin{exemple}
    ...
    pour $E_1$, ça va être l'énergie reçue par le soleil à laquelle on soustrait l'énergie reflétée par la terre, ainsi, en posant $\alpha$ l'albédo de la terre et $Q$ l’énergie reçue par le soleil, 
    \begin{equation*}
        Cd_1 T=E_1-E_2=(1-\alpha)Q-C_2T^4
    \end{equation*}
    où $C_2$ est simplement une constante. Bien sûr, la glace qui se trouve près des pôles reflète bien plus la lumière que l'océan et donc $\alpha$ va diminuer si la température augmente, on peut, par exemple, écrire
    \begin{equation*}
...
Le résultat suivant nous donne un petit critère pour l'existence de telles solutions non triviales, on présente ainsi la \textit{bifurcation simple}, ou encore \textit{bifurcation depuis une branche principale}.
...
\begin{remarque}\label{remarque INd=0}
    Notez que dans les deux cas, on a supposé que l'index de Fredholm était nul. La plupart des théorèmes classiques de bifurcation se placent dans ce cas. La théorie devient beaucoup plus compliquée si on suppose l'index non nul
\end{remarque}
Notez comme les calculs deviennent déjà un peu lourds alors qu'il s'agit de la bifurcation la plus simple ! Ainsi, nous motivons déjà l'approche par \textit{théorie des singularités} que nous exposons dans la section \ref{SINGsection}. Cependant, cette approche suppose qu'on a réussi à se ramener en dimension finie, ça ne marche pas si l'espace des paramètres est de dimension infinie ! d'où la sous-section suivante.
...
\begin{definition}
    Avec les notations habituelles, un zéro $(x_0,\lambda_0)$ de $F(x,\lambda)$ est un \textit{élément de bifurcation}  s'il existe deux suites $(x_i,\lambda_i)_{i\in\mathbb{N}},(x'_i,\lambda'_i)_{i\in\mathbb{N}}$ telles que $x'_i\neq x_i$ pour tous $i\in\mathbb{N}$, convergent vers $(x_0,\lambda_0)$ et 
    $F(x_i,\lambda_i)=F(x'_i,\lambda'_i)=0$ pour tout $i\in\mathbb{N}$. Un élément de bifurcation est dit \textit{perpendiculaire} s'il existe une suite de $X$ $(x_i)_i$ qui tend vers $x_0$ telle que $F(x_i,\lambda_0)=0$ pour tout $i\in\mathbb{N}$
\end{definition}
\begin{remarque}
...
\begin{remarque}\label{remsheaf}
    ...
    \end{itemize}
    Les deux axiomes sont des axiomes purement structurels. Les deux autres permettent de capturer les propriétés désirées. Ainsi,si $Y,X$ sont des variétés lisses alors $C^{\infty}(\cdot,Y): U\mapsto C^{\infty}(U,Y)$ est un faisceau. Et si on définit enfin la \textit{germe} d'un faisceau en un sous-ensemble $S\supseteq X$ comme étant
    \begin{equation*}
...
\begin{definition}
    Un anneau commutatif $R$ est local s'il a un unique idéal maximal. Il est semi local s'il aun nombre fini d'idéaux maximaux
\end{definition}
...
\begin{definition}
    Soit $R$ un anneau, un $R$-module\footnote{Attention, ici on suppose tous les modules unitaires, c'est pas le cas de tous les auteurs. Ainsi, certains ne supposent pas la quatrième condition vraie pour tous les anneaux} à droite est la donnée d'un groupe abélien $G$ est d'une opération "multiplier par un scalaire" $\cdot:R\times G\rightarrow G$ qui respecte les mêmes axiomes que pour un espace vectoriel, i.e, $\forall r,s\in R,x,y\in G$
    \begin{align*}
...
\begin{definition}
    Un module est dit \textit{libre} s'il possède une base. Un module est \textit{finiment généré} s'il possède une partie génératrice finie. Enfin, un module libre est à \textit{rang constant} si toutes les bases ont le même nombre d'éléments
\end{definition}
...
\begin{remarque}
    ...
    finiment généré avec $S=\left\{s\right\}$ est un module \textit{noethérien}, i.e un module dont tous les sous-modules sont finiment générés
\end{remarque}
...
(x_1,\lambda_0),h(\lambda_0))$ et donc,grâce à la structure de $\phi$, elles se trouvent 
...
\begin{definition} 
    $G\in\mathcal{E}^{m+d}_{n+d}\cdot \mathcal{M}_{n+d}$ est  \textit{un déploiement à $d$ paramètres} de $f$
...
\begin{remarque}
    dans la littérature, la notion de déploiement et parfois interchangée avec la notion de \textit{déformation}. Une déformation $F$ de $f$ est simplement une germe de $\mathcal{E}^{m}_{n+d}$ telle que $F(\cdot,0)=f$. En quelque sorte, le déploiement "mémorise" le paramètre mais pas la déformation.
\end{remarque}

\begin{definition}
    Soit $V\subseteq \mathcal{E}^m_n\cdot \mathcal{M}_n$  et $v\in V$ une \textit{famille à $d$-paramètres}  $G$   \textit{ à travers} $v$ \textit{dans $V$} est un élément de  $\mathcal{U}_d(v)$ tel qu'il existe un représentant de  $G_1$, $\tilde{G}_1$ et un voisinage de 0 $U$ tel que $G(u,\cdot)\in V$ si $u\in U$. L'ensemble de tel déploiements sera noté $\mathcal{U}(v,V)$
\end{definition}
...
Plaçons nous, dans un premier temps, en dimension finie et considérons un ouvert $U$ de $\R^n$, $f\in C^0(\bar{U},\R^n)$ et $y\notin f(\partial U)$ et supposons qu'on veuille construire une fonction $\operatorname{deg}$ qui "compte" le nombre de de solutions de l'équation $f(x)=y$ dans $U$. Faisons une petite analyse de la situation: remarquons d'abord qu'on doit avoir la \textit{normalité}
\begin{equation*}
    \forall y\in U,\operatorname{deg}(\operatorname{Id},U,y)=1
\end{equation*}

En effet, dans ce cas la seule solution est $x=y$. Mainenant, si on a deux sous ensembles ouverts de $U$, $V_1$ et $V_2$ et qu'ils sont disjoints. Supposons de plus que toutes les solutions soient dans $V_1$ ou $V_2$ et qu'elles sont en nombre fini, alors on aimerait bien avoir \textit{l'additivité}
...
de façon remarquable, en dimension finie, il n'y a qu'une seule fonction qui possède ces propriétés ! On peut donc parler "du" degré. On l'appelle le \textit{degré de Brouwer}
...
\begin{definition}
    ...
    \end{equation*}
\end{definition}
On peut montrer que cette fonction remplit bien les 3 conditions et que c'est la seule. Ensuite, via le théorème de Morse-Sard et quelques techniques d'approximation classiques, on peut généraliser cette fonction à $C^0(\bar{U})$, voir \cite{KlausNonLin85} pour les détails. Notez que les 3 conditions mènent à une fonction qui ne compte pas vraiment le nombre de solutions, mais associe un \textit{signe} à chacune d'entre elles. Si on voit l'ensemble $f^{-1}(y)$ comme une hypersurface, le signe est positif si on en "sort" et négatif si on "rentre" 
...
\begin{definition}
    Si $X,Y$ sont des espaces de Banach et  $d\in\mathcal{F}_0(X,Y)$ alors $E$ est un \textit{correcteur } de $d$ si $d+E$ est un isomorphisme et si son image est de dimension finie. On pose $\mathcal{C}(d)$ l'ensemble des correcteurs de $d$
\end{definition}
...
\end{lemme}
\begin{preuve}
    Notez que $I-F$ est un isomorphisme, que $(I-F)(X_0)\subseteq X_0$, donc $(I-F)|_{X_0}: X_0\rightarrow X_0$ est un automorphisme d'un espace vectoriel de dimension finie, la notion de déterminant à donc un sens. Supposons d'abord avoir $X_0\subseteq X_1$, on le complémente par $X'_0$, i.e 
    \begin{equation*}
        X_1=X_0\oplus X'_0
    \end{equation*}
...
\begin{definition}
    ...
    \end{equation*}
    où $E_1\in c$ et $E_2\in c'$. La composition de deux opérateurs orientés sera, sauf si mention du contraire, la précédente.
\end{definition}
...
\begin{definition}
    Si $V$ est un espace vectoriel, le \textit{complexifié} $V^{\mathbb{C}}$ de $V$ est donné par 
    \begin{equation*}
...
\end{equation*}
\begin{definition}
    On définit $\sigma(D)$ \textit{le spectre} de $D$ comme étant l'ensemble des complexes $\lambda$ pour lesquels $D$ n'est pas inversible. On décompose ce dernier en trois composantes : 
    \begin{itemize}
        \item le spectre ponctuel $\sigma_p(D)$
        \item le spectre continu $\sigma_c(D)$
        \item le spectre résiduel $\sigma_r(D)$
    \end{itemize}
    on définit enfin \textit{résolvant} comme étant le complémentaire  $\rho(D)$ du spectre.
\end{definition}
