...
\begin{definition}
    Soient $X,Y$ des espaces de Banach, $F:X\rightarrow Y$ telle que de Fredholm en $0$. Considérons alors une base $x_1,\cdots,x_n$ de $\ker DF(0)$ et  $y^{*}_1,\cdots,y^{*}_n$ un base de $\ker{ DF(0)}^{*}$. Le théorème de Hanh-Banach nous assure l'existance d'une complétion bi-orthonogale, i.e, 
    \begin{equation*}
        \exists y_1,\cdots 
    \end{equation*}
\end{definition}
...
La proposition \ref{zeronlinIND} nous suggère de considérer le cas particulier d'un zéro de type $(0,n)$. On complète les $D_2^{*}y_j^{*}\in \Lambda^{*}$ bi-orthonogalement par $(\lambda)_{0\leq j\leq n}$ 
