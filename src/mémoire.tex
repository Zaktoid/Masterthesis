...
\subsection{Opérateurs de Fredholm}
\subsubsection{Définition et théorème d'Atkinson}
...
\begin{remarque}\label{remsheaf}
    C'est un cas particulier de \textit{faisceau}\footnote{Le lecteur familier avec la théorie des catégories ne manquera pas de remarquer qu'un faisceau est un cas particulier de foncteur contravariant de la catégorie des ouverts (dont les morphismes sont les inclusions) et la catégorie des ensembles.}
    Si $(X,\mathcal{T})$ est un espace topologique, un \textit{faisceau} d'ensemble $\mathcal{F}$ sur $X$ est la donnée d'une famille d'ensembles $(\mathcal{F}(U))_{U\in \mathcal{T} }$ et d'une famille de fonction $(\rho^U_V: \mathcal{F}(U)\rightarrow\mathcal{F}(V))_{V\in \mathcal{T}, V\subseteq U}$ respectant les axiomes suivants :
    \begin{itemize}
...
leur constantes). Donc dans la suite, $\dim_{\R} (M/N)$ 
signifiera la dimension de $M/N$ en tant que $\R$-vectoriel.
\\

Les résultats suivants nous permettent de controller le nombre de 
...
\begin{proposition}
    ...
    {n+l}\choose {l}$ où $m$ est la taille du plus petit ensemble de générateurs de $M$
\end{proposition}
...
\begin{equation*}
    ...
    \infty\Rightarrow M \text{ est finiment génré en tant que $\mathcal{E}_y(P)$-module}
\end{equation*}
\end{lemme}
\begin{preuve}
    ...
    \end{equation*}
    or $r$ s'écrit comme une somme de produits finis d'élements de $\pi^{*}(\mathcal{M}_y(P))M$ et $M$, il vient alors qu'on peut écrire
    \begin{equation*}
    ...
    \begin{equation}\label{equationMatrice1}
        A\cdot \begin{pmatrix}
         ...
         =0
    \end{equation}
    où $A_{ij}=t\delta_{ij}-c_{ij}-z_{ij}$. Posons $\Delta=\det A$, via la règle de Kramer, on sait que $\Delta m_i=0$ si $1\leq i\leq q$. On remarque que $-\Delta$ est en réalité le polynôme charactéristique de la matrice $(c_{ij}+z_{ij})$ évalué en $t$, ainsi, 
...
\begin{preuve}
    ...
    $\bigcirc_{i=k+1}^n\pi_i\circ(f,\varphi)^{*}$ pour $k=0$, ça donne juste la structure de  $\mathcal{E}_{y}(P)$-module induite par $f^{*}$. Remarquez que, du fait que $(f,\varphi)$ est une immersion locale,
    \begin{equation*}
        (f,\varphi)^*:\mathcal{E}_{(y,0)}(P\times\R^n)\rightarrow \mathcal{E}_{x}(N)
    \end{equation*}
    Ce qui permet d'affirmer que $M$ est finiment généré en tant que $\mathcal{E}_{(y,0)}(P\times\R^n)$-module.\\
    Supposons maintenant que $M$ soit finiment généré en tant que $\mathcal{E}_{(y,0)}(P\times\R^k)$-module. Notons qu'on a 
    \begin{equation}\label{equation1}
        f^*(\mathcal{M}_y(P))M=(\pi^{*}_{k+1}\circ\cdots \circ \pi^{*}_1)(\mathcal{M}_y(P))M
    \end{equation}
    Où, dans l'expression de gauche, $M$ est vu comme un $\mathcal{E}_x(N)$-module et comme un $\mathcal{E}_{(y,0}(P\times\R^{k+1})$-module dans l'expression de droite. En effet, si $m\in f^*(\mathcal{M}_y(P))M$ alors 
    \begin{equation*}
        m=\sum_{i=1}^{k} f^*(p_i)m_i=\sum_{i=1}^{k} p_i(f_i)m_i
    \end{equation*}
\end{preuve}
...
Nous procédons maintenant au calcul explicite des espaces tangents pour $\mathcal{A}$ et $\mathcal{K}$
