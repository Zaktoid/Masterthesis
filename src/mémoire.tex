...
{\hfill\textcolor{color-prop}{$\clubsuit$}\par}
...
\emph{Master en Sciences Mathématiques, à finalité approfondie/didactique}}

\vfill

{\large Année académique 20??-20??}
...
\begin{definition}
    Soit $V\subseteq \mathcal{E}^m_n\cdot \mathcal{M}_n$  et $v\in V$ une \textit{famille à $d$-paramètres}  $G$   \textit{ à travers} $v$ \textit{dans $V$} est un élément de  $\mathcal{U}_d(v)$ tel qu'il existe un représentant de  $G_1$, $\tilde{G}_1$ et un voisinage de 0 $U$ tel que $[\tilde{G_1}(u,\cdot)]\in V$ si $u\in U$. L'ensemble de tel déploiements sera noté $\mathcal{U}_d(v,V)$
\end{definition}
...
\begin{definition}
    ...
    \end{equation*}
\end{definition}
On peut montrer que cette fonction remplit bien les 3 conditions et que c'est la seule. Ensuite, via le théorème de Morse-Sard et quelques techniques d'approximation classiques, on peut généraliser cette fonction à $C^0(\bar{U})$, voir \cite{KlausNonLin85} pour les détails. Notez que les 3 conditions mènent à une fonction qui ne compte pas vraiment le nombre de solutions, mais associe un \textit{signe} à chacune d'entre elles. 
...
\begin{definition}
    Si $X,Y$ sont des espaces de Banach et  $D\in\mathcal{F}_0(X,Y)$ alors $E\in L(X,Y)$ est un \textit{correcteur } de $D$ si $D+E$ est un isomorphisme et si son image est de dimension finie. On pose $\mathcal{C}(D)$ l'ensemble des correcteurs de $D$
\end{definition}
...
\end{equation*}
répond à la question. En effet, il est injectif car $Y_0\oplus \operatorname*{im} D$ et il est surjectif car une perturbation de rang fini d'un opérateur de Fredholm ne change pas l'indice.
...
\begin{definition}
    dans les conditions des deux lemmes précédents, on dit que les correcteurs  $E$ et  $E'$  sont $D$-\textit{équivalents} si $\det (\phi_{E,E'})|_{X_0}>0$, on écrira  $E\sim_{D} E'$
\end{definition}
\begin{proposition}
...
est un foncteur donc $\mathcal{G}^+(D)$, la catégorie $\mathcal{G}(D)$ ou on retire les les morphismes dont le determinant restreint est négatif, est un sous groupoïde (c'est même un sous groupoïde normal) de $\mathcal{G}(D)$. Il est clair que ce groupoïde à axactement deux composantes connexes
...
\begin{preuve}
    \todo{PREUVE complete}
\end{preuve}
\subsubsection{Applications aux variétés de Banach}
\todo{LEMMES}
\begin{theobox}{Furi}{}
